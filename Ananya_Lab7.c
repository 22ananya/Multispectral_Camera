...
#define DISTANCE_SLOPE_RISE 1.3 //TODO: Calibrate to your own sensor! See Port6 IRQH to see how these are used.
#define DISTANCE_SLOPE_RUN 1
#define DISTANCE_OFFSET -230 //place next to wall and see reading, then set this to that
...
static const float distance_filter_factor=0.1; //for part two

// PID Controller Variables  
#define INITIAL_KP 20
...
static const float control_pwm_scaling=0.1; //to convert control signal into a PWM setting // TODO: tune, up to you

//declaring error terms
static volatile float error_cur;
static volatile float integral_prev;
static volatile float integral_cur;

static volatile float Pterm;
static volatile float Dterm;
static volatile float Iterm;
static volatile float Total;



// Live-Tuning Vars   (automatically toggles between two setpoints on a timer, while allowing you to modify your KP KI KD values
...
#define TUNING_SETPOINT_B 1000
#define TUNING_TIME_PER_STEP 25 //in seconds, max is 32sec.

// Current Measurement Vars
static volatile int16_t current;
static volatile int16_t PWM2;
...
void configTA0forMotorPWM(){
    ...
    TIMER_A0->EX0 = TIMER_A_EX0_IDEX__2; // additional predivider of 8 (64x reduction total)  
...
void driveIndefinitely(bool forward, uint16_t PWM){ // 
    if(forward){
        GPIO_setOutputHighOnPin(GPIO_PORT_P4, GPIO_PIN1 | GPIO_PIN2);
        GPIO_setOutputLowOnPin(GPIO_PORT_P4, GPIO_PIN3 | GPIO_PIN4);
    } else {
        GPIO_setOutputLowOnPin(GPIO_PORT_P4, GPIO_PIN1 | GPIO_PIN2);
        GPIO_setOutputHighOnPin(GPIO_PORT_P4, GPIO_PIN3 | GPIO_PIN4);
    }
...
void updatePIDController(){
    ...
    distance_mm_intoPI = distance_mm*distance_filter_factor + (1-distance_filter_factor)*distance_mm_intoPI;

    //Todo: calc P, D, and I terms using the global K constants and relevant global sensor output values
    error_cur = (distance_mm_intoPI-setpoint)/10; //scaling the error so that kp when 20 multiplies by an error of like 4 to give a pwm2 value of 80
    integral_prev = integral_cur;
    integral_cur = i_decay_rate*error_cur+(1-i_decay_rate)*integral_prev;
    Pterm = k_p*(error_cur);
    Dterm = k_d*enc_curspeed/40000000;
    Iterm = k_i*integral_cur;
    Total = Pterm + Dterm + Iterm;

    /*k_p = 10000/500; //change the numerator 10000 accordingly
    k_d = 500;
    k_i = 1000;*/
    ...
    driveIndefinitely(driveCommand_forward, PWM2);
...
void PORT6_IRQHandler(){
    ...
    if(status & GPIO_PIN0){
        if(distance_riseTimestamp == 0){ // we haven't seen the rising edge yet this cycle
            ...
            distance_mm = DISTANCE_SLOPE_RISE * (distance_highTime - DISTANCE_OFFSET )/DISTANCE_SLOPE_RUN ; // 
            ...
            //  (updates the P and I terms, off of the new distance value)
            updatePIDController();
        }
    }
}
